...
public class Movement : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        if (pressedDown)
        {
            ...
            {
                BhapticsLibrary.PlayParam(BhapticsEvent.BACKJET, -yValue * 0.5f, 1f, 0f, 0f);
                BhapticsLibrary.PlayParam(BhapticsEvent.RIGHTJET, -xValue, 1f, 0f, 0f);
            }
            else if (xValue <= 0 && yValue >= 0)
            {
                BhapticsLibrary.PlayParam(BhapticsEvent.FRONT, yValue * 0.5f, 1f, 0f, 0f);
                BhapticsLibrary.PlayParam(BhapticsEvent.RIGHTJET, -xValue, 1f, 0f, 0f);
            }
            else if (xValue >= 0 && yValue >= 0)
            {
                BhapticsLibrary.PlayParam(BhapticsEvent.FRONT, yValue*0.5f, 1f, 0f, 0f);
                BhapticsLibrary.PlayParam(BhapticsEvent.LEFTJET, xValue, 1f, 0f, 0f);   
            }
            else if (xValue >= 0 && yValue <= 0)
            {
                BhapticsLibrary.PlayParam(BhapticsEvent.BACKJET, -yValue * 0.5f, 1f, 0f, 0f);
    ...
    // void NullifyMovement(InputAction.CallbackContext context)
    // {
    //     Debug.Log("PULSAO CANCELAO");
